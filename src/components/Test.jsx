...
import { convertToSec, calculateMinCo2Value } from '../helpers/Functions.js';
...
const Test = () => {
    ...
    const fromDestination = state
        ? JSON.parse(state.fromDestination)
        : destinations.miscDestinations.find((destination) => destination.name === 'Munich');
    const toDestination = state && JSON.parse(state.toDestination);
    const transports = state ? JSON.parse(state.transports) : ['Flight ✈️', 'Driving 🚗', 'Train 🚆'];
                            ...
                            <Paper sx={{ overflow: 'auto', height: '70vh' }}>
                                {toDestinations.slice(startIndex, endIndex).map((toDest, index) => (
                                    <Box key={index} sx={{ mb: 3 }}>
                                        <DestinationListItem
                                            destination={toDest}
                                            minCo2Value={calculateMinCo2Value(fromDestination, toDest)}
